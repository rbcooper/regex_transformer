...
# %%
def states_to_one_hot_locations(
    ...
    locations = t.zeros(
        batch_size, states_len, n_probes, n_probe_outputs, device=device
    )
...
for i, (tokens, states) in tqdm.auto.tqdm(enumerate(dl), total=n_batches):
    ...
    states_one_hot = states_to_one_hot_locations(
        states
    ).cuda()  # (batch, pos, cubie_id, cubie_loc)
    ...
    probe_correct_log_probs = (
        probe_log_probs * states_one_hot
    )  # (batch, pos, cubie_id, cubie_loc)
    average_correct_log_probs = (
        reduce(
            probe_correct_log_probs, "batch pos cubie_id cubie_loc -> cubie_id", "mean"
        )
        * n_probes
    )  # Take the *mean* over batch, pos and *sum* over cubie locations
    ...
    if i % 20 == 0:
        ...
        accuracy = list(
            probe_log_probs.argmax(-1)
            .eq(cubie_locations_integer)
            .float()
            .mean((0, 1))
            .cpu()
            .numpy()
        ...
        # })
        print(
            f"loss={loss.item():.3f}, accuracy=[{' '.join(f'{a:.3f}' for a in accuracy)}]"
        )
# wandb.finish()
...
loss_components_by_position = reduce(
    loss_components, "pos cubie_id cubie_loc -> pos cubie_id", "mean"
)
...
loss_components_by_cubie_id = reduce(
    loss_components, "pos cubie_id cubie_loc -> cubie_id", "mean"
)
# Make a bar graph
px.bar(
...
loss_components_by_cubie_id_and_cubie_loc = reduce(
    loss_components, "pos cubie_id cubie_loc -> cubie_id cubie_loc", "mean"
)
...
for layer in range(model.cfg.n_layers):
    ...
    attn_scores_by_pos = reduce(
        cache["attn_scores", layer].cpu().numpy(),
        "batch n_heads pos_from pos_to -> pos_from pos_to",
        "mean",
    )
    # Set minimum to nan
    # mask =
