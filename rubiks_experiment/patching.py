...
def get_reappearance_allowed_rotations_fn(leaving_move = 10, reappearing_move = 20):
    ...
    def reappearance_allowed_rotations_fn(states: List[CubieRepresentation], rotations: List[CubeMove], this_rotation = CubeMove) -> bool:
        """
        ...
        if n < leaving_move or n > reappearing_move:
            ...
            print(f"Sticker {rot_to_leaving_sticker_dict[this_rotation]} is leaving at move {n} ({this_rotation})")
            ...
            sticker_id = cubie_id * 3 + states[leaving_move - 1].cubie_rotations[cubie_id][2] # z axis after move 9
            print(f"Sticker ID: {sticker_id} on cubie {cubie_id}, color {rubiks_generator.cube_colors[states[leaving_move - 2].sticker_colors[sticker_id]]}")
            print(f"Sticker coming from {states[leaving_move - 1].get_position_of_sticker_id(sticker_id)}")
            print(f"Sticker going to {states[leaving_move].get_position_of_sticker_id(sticker_id)}")
            # if ret:
            ...
            if n == reappearing_move - 1:
                ...
                ret = z == 1 and axis == 2
                if ret: print(f"Sticker returning with move {this_rotation} to {x},{y},{z} with axis {axis}")
            else:
                # Ensure the sticker is not on the up face
                ret = not (z == 1 and axis == 2)
            if ret: print(f"At move {n} after rotation {this_rotation}, sticker {sticker_id} is at {x},{y},{z} with axis {axis}")
            return ret
...
# %%
rng = np.random.default_rng(0)
reappearance_data = rubiks_generator.generate_222_cube_data(125, rng, allowed_rotations_fn=get_reappearance_allowed_rotations_fn(1, 3))
# %%
